...
if (!empty($setmodules)) {
    ...
    $module[_l('Item Shop Admin')][_l('ADD Cash Shop')] = $file;
    return;
}

$lefttitle = _l('Item Shop Admin - ADD Cash Shop');;
$time = date('F j Y G:i');
...
if ($this_script == $script_name) {
    ...
    if (hasPermissions($do)) {
        ...
        if ($update != '') {
            ...
            $Cash = (isset($_POST['Cash']) && is_array($_POST['Cash'])) ? $_POST['Cash'] : '';
            ...
            if ($serial != '' && $Cash != '') {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $userpoint = (is_int((int)$Cash[$i])) ? antiject((int)$Cash[$i]) : '';

                    if ($userid != '' && $userpoint != '') {
                        connectcashdb();
                        $select_user = "SELECT Cash,id FROM BILLING.dbo.tbl_UserStatus WHERE serial = '$userid'";
                        $select_result = mssql_query($select_user);
                        $user = mssql_fetch_array($select_result);

                        if ($user['Cash'] != $userpoint) {

                            $update_points = "UPDATE tbl_UserStatus SET Cash = '" . $userpoint . "' WHERE serial = '" . $userid . "'";
                            if (!($update_points_result = mssql_query($update_points))) {
                                $exit_process = 1;
                                $out .= '<p style="text-align: center; font-weight: bold;">Unable to update user cash coin!</p>';
                            }

                            // Writing an admin log :D
                            gamecp_log(0, $userdata['username'], "ADMIN - ADD CASH SHOP - UPDATED - Account ID: " . $user['id'] . " | New Cash: $userpoint | Old Cash: " . $user['Cash'], 0);
                ...
                if ($exit_process != 1) {
                    $out .= '<table class="table table-bordered" align="center">' . "\n";
                    $out .= '	<tr>' . "\n";
                    $out .= '		<td class="alt1" style="text-align: center;">Updated user cash coin!</td>' . "\n";
                    $out .= '	</tr>' . "\n";
                    $out .= '</table>' . "\n";
                    $out .= '<br/>' . "\n";
                }
        ...
        if ($search_fun != "") {
            ...
            if ($account_name == '') {
                ...
                connectcashdb();
                $sql = "SELECT Serial FROM BILLING.dbo.tbl_UserStatus WHERE id = '" . $account_name . "'";
        ...
        if ($exit_process == 0) {
            ...
            $out .= '		<td class="thead">Cash Coin</td>' . "\n";
            $out .= '	</tr>' . "\n";
            ...
            # SQL Statements
            $query_p1 = "SELECT serial, Cash, id FROM BILLING.dbo.tbl_UserStatus" . $search_query;
            ...
            while ($row = mssql_fetch_array($rs)) {
                ...
                $out .= '		<td class="alt1"><input type="text" class="form-control" name="Cash[]" value="' . $row['Cash'] . '" /></td>' . "\n";
                $out .= '	</tr>' . "\n";
            ...
            if (mssql_num_rows($rs) > 0) {
                ...
                $out .= '</tr>' . "\n";
                $out .= '<tr>' . "\n";
                $out .= '	<td class="alt2" colspan="3" style="text-align: center;"><input type="hidden" name="account_name" value="' . $account_name . '"/><input type="submit"  class="btn btn-default" name="update" value="Update Cash" /></td>' . "\n";
                $out .= '</tr>' . "\n";
            } else {
                $out .= '<tr>' . "\n";
