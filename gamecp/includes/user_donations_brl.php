...
if (!empty($setmodules)) {
    ...
    $module[_l('Donations')][_l('Donate for Game Point Credit Card(BRL)')] = $file;
    return;
}
...
# Set the page title
$lefttitle = "Donate for Game Point Credit Card (BRL)";
...
if ($this_script == $script_name) {
    ...
    if (isset($isuser) && $isuser == true) {
        ...
        # IPN URL/
        $ipn_url = (isset($config['paypal_ipn_brl_url'])) ? $config['paypal_ipn_brl_url'] : '';

        # Ensure that we have a valid PayPal IPN URL
        if (filter_var($ipn_url, FILTER_VALIDATE_URL) === FALSE) {
            $url = get_url() . 'paypal_ipn_brl.php';
            $out .= '<p style="text-align: center; font-weight: bold;">' . _l('Você inseriu um URL IPN do PayPal inválido. Por favor, defina-o para %s', $url) . '</p>';
            return 0;
        }
        ...
        if ($username == '') {
            ...
            if (!($result = mssql_query($select_query))) {
                $exit_stage = 1;
                $out .= '<p style="text-align: center; font-weight: bold;">Erro de SQL ao tentar consultar o banco de dados</p>';
            }
        ...
        if ($exit_stage == 0) {
            ...
            $out .= '	Se o seu Item de Compra de Créditos e para um amigo, insira o seu <b><u>username</u></b> abaixo.<br /><br />' . "\n";
            $out .= '	<input type="text" name="username"> <input type="submit" class="submit" value="Change">' . "\n";
            $out .= '</form>' . "\n";
            $out .= '</center>' . "\n";
            $out .= '<br>' . "\n";

            calculate_credits($config['donations_credit_muntiplier'], $config['donations_number_of_pay_options'], $config['donations_start_price_brl'], $config['donations_start_credits_brl'], false);

            $out .= '<table class="tborder" cellpadding="3" cellspacing="1" border="0" width="60%" align="center">' . "\n";
            if ($username != '') {
                $out .= '		<tr>' . "\n";
                $out .= '			<td colspan="5" style="text-align: center; font-size: 15px;">Você está comprando esses créditos para <b>' . $username . '</b></td>' . "\n";
                $out .= '		</tr>' . "\n";
            }
            $out .= '		<tr>' . "\n";
            $out .= '			<td class="thead">Preço</td>' . "\n";
            $out .= '			<td class="thead">Creditos</td>' . "\n";
            $out .= '			<td class="thead">Bonus</td>' . "\n";
            $out .= '			<td class="thead" style="text-align: center;">Total</td>' . "\n";
            $out .= '			<td class="thead" style="text-align: center;">Doe Agora!</td>' . "\n";
            $out .= '		</tr>' . "\n";
            for ($i = 1; $i < count($c_price); $i++) {
                $bgcolor = ($i % 2) ? 'alt1' : 'alt2';

                $out .= '	<tr>' . "\n";
                $out .= '		<td class="' . $bgcolor . '">R$' . number_format($c_price[$i], 2, '.', '') . '</td>' . "\n";
                ...
                $out .= '		<td class="' . $bgcolor . '" align="center"><form action="https://www.paypal.com/cgi-bin/webscr" method="post">' . paypal_buttons_brl(number_format($c_price[$i], 2, '.', ''), $c_total[$i], $custom) . '</form></td>' . "\n";
                $out .= '	</tr>' . "\n";
            }
            $out .= '	</table>';
