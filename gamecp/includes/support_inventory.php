...
if (!empty($setmodules)) {
    ...
    $module[_l('Support')][_l('Inventory Search')] = $file;
    return;
}

$lefttitle = _l('Support Desk - Inventory Search');;
$time = date('F j Y G:i');
...
if ($this_script == $script_name) {
    ...
    if (hasPermissions($do)) {
        ...
        if (empty($page)) {
            ...
            $out .= '		<td class="alt2"><input type="text" class="form-control" name="item_amount_min" size="1" /> - <input type="text" class="form-control" name="item_amount_max" size="1" /></td>';
            $out .= '	</tr>';
            $out .= '	<tr>';
            $out .= '		<td class="alt1">Upgrades:</td>';
            $out .= '		<td class="alt2">' . "\n";
            $out .= '<select class="form-control"name="item_ups">';
            for ($x = 0; $x <= 7; $x++) {
                $select_ups = "";
                if ($item_ups == $x) {
                    $select_ups = " selected";
            ...
            $out .= '</select>';
            $out .= '/';
            $out .= '<select class="form-control"name="item_slots">';
            for ($y = 0; $y <= 7; $y++) {
                $select_slots = "";
                if ($item_slots == $y) {
            ...
            $out .= '</select>';
            $out .= ' ';
            $out .= '<select class="form-control"name="item_talic">';
            for ($z = 1; $z < 16; $z++) {
