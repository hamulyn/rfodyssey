...
if (!empty($setmodules)) {
    ...
    $module[_l('Donations')][_l('Donate for Game Point Credit Card (PHP)')] = $file;
    return;
}
...
# Set the page title
$lefttitle = "Donate for Game Point Credit Card (PHP)";
...
if ($this_script == $script_name) {
    ...
    if (isset($isuser) && $isuser == true) {
        ...
        # IPN URL/
        $ipn_url = (isset($config['paypal_ipn_php_url'])) ? $config['paypal_ipn_php_url'] : '';

        # Ensure that we have a valid PayPal IPN URL
        if (filter_var($ipn_url, FILTER_VALIDATE_URL) === FALSE) {
            $url = get_url() . 'paypal_ipn_php.php';
        ...
        if ($exit_stage == 0) {
            ...
            calculate_credits($config['donations_credit_muntiplier'], $config['donations_number_of_pay_options'], $config['donations_start_price_php'], $config['donations_start_credits_php'], false);
            ...
            for ($i = 1; $i < count($c_price); $i++) {
                ...
                $out .= '	<tr>' . "\n";
                $out .= '		<td class="' . $bgcolor . '">PHP$' . number_format($c_price[$i], 2, '.', '') . '</td>' . "\n";
                ...
                $out .= '		<td class="' . $bgcolor . '" align="center"><form action="https://www.paypal.com/cgi-bin/webscr" method="post">' . paypal_buttons_php(number_format($c_price[$i], 2, '.', ''), $c_total[$i], $custom) . '</form></td>' . "\n";
                $out .= '	</tr>' . "\n";
            }
            $out .= '	</table>';
