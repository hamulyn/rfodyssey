...
if (!empty($setmodules)) {
    ...
    $module[_l('Item Shop Admin')][_l('ADD Vote Point')] = $file;
    return;
}

$lefttitle = _l('Item Shop Admin - Vote Point');;
$time = date('F j Y G:i');
...
if ($this_script == $script_name) {
    ...
    if (hasPermissions($do)) {
        ...
        if ($update != '') {
            ...
            $user_point = (isset($_POST['user_vote_points']) && is_array($_POST['user_vote_points'])) ? $_POST['user_vote_points'] : '';
            ...
            if ($user_id != '' && $user_point != '') {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if ($userid != '' && $userpoint != '') {
                        connectgamecpdb();
                        $select_user = "SELECT user_vote_points,user_account_id FROM gamecp_gamepoints WHERE user_id = '$userid'";
                        $select_result = mssql_query($select_user);
                        $user = mssql_fetch_array($select_result);

                        if ($user['user_vote_points'] != $userpoint) {

                            $update_points = "UPDATE gamecp_gamepoints SET user_vote_points = '" . $userpoint . "' WHERE user_id = '" . $userid . "'";
                            ...
                            gamecp_log(0, $userdata['username'], "ADMIN - ADD VOTE POINTS - UPDATED - Account ID: " . $user['user_account_id'] . " | New Points: $userpoint | Old Points: " . $user['user_vote_points'], 0);
        ...
        if ($exit_process == 0) {
            ...
            # SQL Statements
            $query_p1 = "SELECT user_id, user_vote_points, user_account_id FROM gamecp_gamepoints" . $search_query;
            ...
            while ($row = mssql_fetch_array($rs)) {
                ...
                $out .= '		<td class="alt1"><input type="text" class="form-control" name="user_vote_points[]" value="' . $row['user_vote_points'] . '" /></td>' . "\n";
                $out .= '	</tr>' . "\n";
